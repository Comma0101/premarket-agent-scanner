...
| Catalyst/news quality | Breaking-news movers need source, freshness, headline substance, and relevance checks before a catalyst label is trusted. | partial: RSS discovery covers PRNewswire, GlobeNewswire, and Business Wire with conservative ticker matching, cache persistence, `hard`/`soft`/`unknown` quality labels, and derived recency minutes during enrichment; absence still means unknown, not "no catalyst" | Broader News API, Benzinga, Financial Modeling Prep news, SEC press releases, issuer investor-relations feeds, or additional curated RSS feeds |
| SEC filing / offering / dilution risk | Penny-stock and small-cap moves can fail or become dangerous when recent filings indicate offerings, toxic financing, reverse splits, or heavy dilution risk. | partial: recent EDGAR forms are cached and mapped to filing-risk tags when CIK resolution succeeds; this is not a complete dilution, warrant, ATM, or toxic-financing parser | SEC EDGAR company filings, FMP SEC filings, Nasdaq filings, issuer investor-relations filing feeds, or purpose-built dilution parsing |
| Former-runner history | Former runners require historical spike dates, prior percent moves, volume surges, and prior catalyst context, not just today's gap. | partial: local runner-history records can be attached and surfaced, but the project still needs a first-class historical scan archive to populate them broadly | Local historical scan archive, Polygon aggregates, yfinance history, Nasdaq historical data, or manually curated watchlists |
...
2. Broaden catalyst/news coverage and harden SEC filing/dilution parsing.
3. Build first-class former-runner archives and intraday pattern history.
