...
class MockBarProvider:
    ...
    def get_bars(self, ticker: str, timeframe: str, start: str | None = None, end: str | None = None, limit: int = 100) -> IntradayBarSeries:
        if timeframe == "1Day":
            return IntradayBarSeries(ticker, timeframe, self.daily_bars, self.source_name, "2026-06-29T14:00:00Z")
        return IntradayBarSeries(ticker, timeframe, self.intraday_bars, self.source_name, "2026-06-29T14:00:00Z")


def _make_bar(ticker: str, c: float, h: float = 10.0, low_val: float = 9.0, o: float = 9.5, tf: str = "1Day", ts: str = "2026-06-25T14:00:00Z") -> IntradayBar:
    return IntradayBar(
        ticker=ticker,
        timestamp=ts,
        open=o,
...
def test_detect_first_red_day_valid():
    ...
    intraday = [
        _make_bar("AAPL", 13.5, h=14.0, low_val=13.1, tf="2Min", ts="2026-06-25T13:30:00Z"),
        _make_bar("AAPL", 12.9, h=13.5, low_val=12.5, tf="2Min", ts="2026-06-25T13:32:00Z"),  # breakdown below 13.0
    ]
    ...
    assert signal.entry_price == 12.99
...
def test_reject_not_enough_green_days():
    ...
    intraday = [
        _make_bar("AAPL", 12.9, h=13.5, low_val=12.5, tf="2Min", ts="2026-06-25T13:32:00Z"),
    ]
    svc = TemizAnalysisService(MockBarProvider(daily, intraday))
